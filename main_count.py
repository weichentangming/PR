...
def parse_args():
    ...
    parser.add_argument('--glove_file_path', type=str, default='/hhd12306/chentangming/dataset/models/glove/glove.6B.300d.txt')
    parser.add_argument('--feat_category',type=str,default='resnet')
    parser.add_argument('--feat_path',type=str,default='/hhd12306/chentangming/dataset/tgif/features')
    parser.add_argument('--Multi_Choice',type=int, default=5)
    parser.add_argument('--test_phase', type=bool, default=True)
    # parser.add_argument('--scale', default=[256, 512, 1024])
    # parser.add_argument('--scale', default=[1024])

    parser.add_argument('--scale', default=[1024, 512, 256])
...
if __name__ == '__main__':
    ...
    if not args.test_phase:
        ...
        test_dset = VQAFeatureDataset(args.task, dictionary, args.sentense_file_path,args.feat_category,args.feat_path, mode='Test')
        batch_size = args.batch_size
        model_name = args.task + '_model'
        model = getattr(locals()[model_name], 'build_%s' % args.model)(args.task, test_dset, params).cuda()
        # model.w_emb.init_embedding(dictionary, args.glove_file_path, args.task)
        print('========start test========')
        ...
        model = model.cuda()
        model.train(False)
        test_loader = DataLoader(test_dset, 16, shuffle=False, num_workers=1)
