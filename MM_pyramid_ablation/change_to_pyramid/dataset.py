...
class VQAFeatureDataset(Dataset):
    def __init__(self, task, dictionary, dataroot='data', feat_category='resnet', feat_path='./data/',mode='Train', Mul_Choice=5):
        ...
        self.max_length = 36
        if os.path.exists('/hhd12306/chentangming/dataset/tgif/features/bert_6/%s_%s_entries_info.pkl'%(task, mode)):
            print('exsit path')
            self.entries = pkl.load(open('/hhd12306/chentangming/dataset/tgif/features/bert_6/%s_%s_entries_info.pkl'%(task, mode), 'rb'))
        else:
            self.entries = _load_dataset('./data', mode, task, self.ans2label)
            self.tokenize()
            with open('/hhd12306/chentangming/dataset/tgif/features/bert_6/%s_%s_entries_info.pkl'%(task, mode), 'wb') as f:
    ...
    def tokenize(self, max_length=35):
        ...
        if self.task.lower() in mc_task:
            for entry in self.entries:
                ...
                for i, candi in enumerate(['a1','a2','a3','a4','a5']):
                    ...
                    with torch.no_grad():
                        ...
                        q_embeds.append(np.array(encoded_layers[5].cpu().squeeze()))
            ...
            for entry in self.entries:
                ...
                with torch.no_grad():
                    encoded_layers, _ = model(token_tensor, mask)
                    entry['q_embed'] = np.array(encoded_layers[5].cpu().squeeze())
