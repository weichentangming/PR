...
class ParalCoAttention(nn.Module):
    ...
    def forward(self, v, q, v_mask=True):
        ...
        for i in range(self.R):
            pyramid_v.append(self.v_list[i](v))
            pyramid_q.append(self.q_list[i](q))
            v = pyramid_v[i]
            q = pyramid_q[i]
        length = self.R
        for i in range(self.R):
            ...
            h_v = pyramid_v[length - i - 1]
            h_q = pyramid_q[length - i - 1]
            ...
            v_attend = (v_initial * v_att)  # bs x dim
            q_attend = (q_initial * q_att)
            v_atts.append(v_attend)
            q_atts.append(q_attend)
            q_states += q_attend
            v_states += v_attend
        q_states = q_states + q_initial
        v_states = v_states + v_initial
