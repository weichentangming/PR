...
import numpy as np
from models.attention import paraAttention, ParalCoAttention, CoAttention
...
class ActionModel(nn.Module):
    ...
    def sample(self, dataset, dataloader):
        ...
        for v,q,a in iter(dataloader):
            ...
            q = torch.from_numpy(q.reshape(-1, q.shape[-1]))
            v = np.array(v)
            v = np.tile(v, [1, 5]).reshape(-1, v.shape[-2], v.shape[-1])
            v = Variable(torch.from_numpy(v).cuda())
            q = Variable(q.cuda())
            ...
            gruth_label = dataset.entries[j]['label']

            pred_answer = dataset.entries[j]['a' + str(pred_label+1)]
            gruth_answer = dataset.entries[j]['a' + str(gruth_label+1)]
            ...
            results.append(
                {'key':key,
                 ...
                 'gt_answer': gruth_answer,
                 'pred_label': pred_label,
                 'gt_label': gruth_label,
                 'j':j
                 }
            )
            j = j + 1
        json.dump(results, open('Action.json', 'w'))
...
def build_temporalAtt(task_name, dataset, params):
    ...
    v_fusion_att = paraAttention(fuse_dim=dataset.v_dim, glimpses=params['sub_nums'], inputs_dim=dataset.v_dim,
                                 att_dim=num_hid)
    ...
    return ActionModel(task_name, q_proj, co_att, q_fusion_att, v_fusion_att, classifier)
...
def build_ParalCoAtt(task_name, dataset, params):
    ...
    return ActionModel(task_name, q_proj, co_atts, q_fusion_att, v_fusion_att, context_gate, classifier)
