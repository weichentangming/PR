...
def train(model, train_loader, eval_loader, params, opt=None):
    ...
    for epoch in range(params['epochs']):
        ...
        for i, (v, q, q_embed, a) in enumerate(train_loader):
            ...
            # q = Variable(q.cuda())
            q = q.reshape(-1, q.shape[-1]).cuda()
            q_embed = q_embed.reshape(-1, q_embed.shape[-2], q_embed.shape[-1]).cuda()
            v = v.repeat(1, 1, model.num_choice).reshape(-1, v.shape[-2], v.shape[-1]).cuda()
            ...
            if i % 100==0:
                ...
                # print('iter: ', i, 'loss: %6f' % loss.item())
